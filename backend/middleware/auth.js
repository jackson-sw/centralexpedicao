...
// (dono original da tela), Expedição (que também monta caixa quando
// precisa, direto do pátio) e Produção (idêntico a Almoxarifado, só
// com equipe própria — ver RESPONSAVEIS_POR_PERFIL em constants.js).
// NÃO inclui Expedição Administrativo: esse perfil não tem impressora
// Argox configurada (ver permiteImprimirEtiqueta no frontend e
// IMPRESSORA_POR_PERFIL em backend/routes/etiquetas.js), então o
// botão "Nova Caixa" nem aparece pra ele.
function apenasMontagemCaixa(req, res, next) {
  if (!['almoxarifado', 'expedicao', 'producao'].includes(req.usuario?.perfil)) {
    return res.status(403).json({ erro: 'Acesso restrito aos perfis Almoxarifado, Expedição e Produção.' });
  }
  next();
}
